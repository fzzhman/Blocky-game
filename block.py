...
class Block:
    ...
    def smash(self) -> bool:
        ...
        if self.smashable():
            if rand < math.exp(-0.25 * self.level):
                positions = self._children_positions()
                size = self._child_size()
                level = self.level + 1
                for i in range(4):
    ...
    def _majority_colour(self) -> Optional[Tuple[int, int, int]]:
        ...
        """
        colours = [[self.children[0].colour, 0]]
        for child in self.children[1:]:
            for set_ in colours:
                if child.colour in set_:
                    set_[1] += 1
                else:
                    colours.append([child.colour, 0])
        t_f = []
        for colour in colours:
            max_ = True
            for colour2 in colours:
                if colour[1] <= colour2[1]:
                    max_ = False
            t_f.append(max_)

        if True in t_f:
            index = t_f.index(True)
