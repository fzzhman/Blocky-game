...
def _find_max_level(block: Block) -> int:
    """Return the level of the deepest block inside this block.
    """
    if len(block.children) == 0:
        return block.level
    else:
        child0 = _find_max_level(block.children[0])
        child1 = _find_max_level(block.children[1])
        child2 = _find_max_level(block.children[2])
        child3 = _find_max_level(block.children[3])
        return max(child0, child1, child2, child3)
...
def _get_block(block: Block, location: Tuple[int, int], level: int) -> \
    ...
    Preconditions:
        - 0 <= level <= max_depth
    """
    max_level = _find_max_level(block)

    if block.level == level or (level > max_level and block.level == max_level):
        size = block.size
...
class RandomPlayer(Player):
    ...
    def generate_move(self, board: Block) ->\
        ...
        return _generate_random_valid_moves(board, self.goal)
...
def _generate_random_valid_moves(board: Block, goal: Goal) ->\
    ...
    level = random.randint(0, board.max_depth+1)
...
class SmartPlayer(Player):
    ...
    def _get_score(self, board: Block, block: Block,
        ...
        """Return the score of the board when the move is performed on the
        given block.
        """
    ...
    def generate_move(self, board: Block) ->\
        ...
        for move in valid_moves:
            block = move[2]
            m = (move[0], move[1])
            s = self._get_score(board, block, m)
            scores.append(s)
